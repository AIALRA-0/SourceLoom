...
模型收到规则不等于已经遵守；新流程分别记录原件与结构检查、当前批次模型核对、按风险触发的跨批次语义核对和用户接受，保存成功不会被标成绝对无损证明
...
公开配置示例不包含生产环境的夸父社价格快照或密钥；实际线路与价格从服务器端设置读取

设置窗口管理普通主通道；服务器可以用 `role_provider_overrides` 固定少数不兼容角色的备用通道，切换设置或重启后台任务都不会清除这些覆盖
