...
def isolated_intake(store, uploads, source_url=None, timeout=75):
    ...
    request={'root':str(store.root),'uploads':[(n,store.blob(r)) for n,r in uploads],'source_url':source_url}
...
def main():
    ...
    try:
        result={'inventory':intake(store,[(n,store.read_blob(k)) for n,k in request['uploads']],request['source_url'])}
