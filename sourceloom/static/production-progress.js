...
export function progressMarkup(run){
 ...
 const content=`<div class="production-progress"><ol aria-label="处理阶段">${steps}</ol><p role="status">${escape(p.label)}${p.active?'，可以关闭页面后再回来':''}</p>${p.active?`<span class="elapsed" data-started="${Number(p.started)||0}"></span>`:''}${error}${originals}</div>`;
 return p.active?content:`<details class="processing-summary"><summary>处理记录<span>${escape(p.label)}</span></summary>${content}</details>`;
}
