...
1. 复用 ReadWeave 已配置的夸父社、TinyFish、OpenAlex、Octen 和 Parallel 等接口，默认以夸父社的深度求索模型承担生成，并把其他接口限制在适合的检索或备用位置。
...
| 能力 | 默认职责 | 初始策略 |
| --- | --- | --- |
| 夸父社 `deepseek-v4.1-flash` | 规划、写作、审核和局部修复 | 默认生成通道，使用 Responses 协议 |
...
最终交付不是中间日志或抽象评分，而是：可访问的生产版本、八份从未测试过的真实输入及其最终输出、逐篇费用、完整调用明细、可复现的检查报告、GitHub 提交和部署版本。用户随后可以直接上传自己的材料进行现场体感测试。
